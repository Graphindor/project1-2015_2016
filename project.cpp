...
void evil_power(int k, int i, adjacency_list_t &adjacency_list, vector<vertex_t> &best_path, int best_dist, int &minimum_dist, vector<weight_t> &min_distance, vector<vertex_t> &previous)
{
        ...
        if(adjacency_list[best_path[j]].size() > 1) /*&& path[j] != entrances[i]*/// && ContainsPair(v,make_pair(best_path[j], best_path[j + 1])) == false)
        {
            ...
            for(int k = 0; k < adjacency_list[best_path[j]].size() && evil_found == false; k++)
            {
                ...
                */
                ...
                cout << "evil_power with k => " << k << endl;

                if(k > 0)
                {
                    evil_power(k - 1, i, adjacency_list, my_path, best_dist, minimum_dist, min_distance, previous);
                }
                else
                {
...
int main()
{
    ...
    for(int i = 0; i < entrances.size(); i++)
    {
        ...
        if(K > 0)
        {
            evil_power(evil_moves, i, adjacency_list, best_path, best_dist, minimum_dist, min_distance, previous);
        }
        ...
        cout << ">>>>>>>>>>>>>>>>>>>Pushing back answer => " << answers.back() << "<<<<<<<<<<<<<<<<<<<<<<<<" << endl;
    }
